...
{
    [CustomPropertyDrawer(typeof(WorldStateEntry))]
    public class WorldStateEntryDrawer : PropertyDrawer
    {
        private WorldStateCollection _collection;
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (_collection == null)
            {
                _collection = WorldStateCollection.Load();
                if (_collection == null)
                {
                    GUI.color = Color.red;
                    EditorGUI.LabelField(position, "No World State Collection Found.");
                    return;
                }
            }

            var att = (WorldStateEntry)attribute;
